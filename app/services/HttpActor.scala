...
class HttpActor extends Actor with ActorLogging {
  ...
  lazy val mapUrl = "https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyDhVNg3fi2qfu3JrW-XBjRVmkv6PiDw8jg&components=country:RU|administrative_area_level_2:g.+Krasnodar&bound=44.90,38.80|45.20,39.20&address="
  ...
  def getLocation(addr: String): Location = {
    val transAddr = Transliterator.translit(addr)
    val url = mapUrl + mapArea + transAddr.replaceAll("\\s+", "+")
